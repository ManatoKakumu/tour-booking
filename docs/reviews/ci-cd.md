...
一方で、壁打ちの中で既に方針として決着している内容の一部が`ci-cd.md`に反映されておらず、テンプレートの必須セクション(可用性設計・コストの見積もり)も空欄のままです。特にOIDC用IAM Roleの権限粒度は、これまでB/C分離で一貫してきた「侵害時の被害限定」という設計原則をCI/CDの認証情報にも適用するかどうかという設計判断そのものなので、書き忘れではなく一度考えてから埋めてください。設計レビューとしてはここで差し戻し、Terraform実装には進まないでください。
...
- 抜け: 「CI/CD方針確定・DBユーザー作成の置き場所」として既に決着していた内容(2026-08-17時点) — `api-b`/`api-c`のOIDC権限を使った一度限りのECS RunTaskでDBユーザー作成SQLを実行する、`workflow_dispatch`の別ジョブとして用意する — が`ci-cd.md`に一切出てこない。今回のスコープがコンピュート非依存部分のみだとしても、決定済みの設計判断なら「この部分はコンピュート層構築時に追記する」のような一文を残した方が、後から読んだときに「決めていない」のか「対象外」なのか判別できる
